...
The full background and text of the story will be researched by a writer of the magazine. You will be using the 2017 wave of the HCMST survey - provided as `HCMST_couples.rds`. The file `HCMST_variable_descriptions.pdf` contains most of the variable descriptions and coding of responses.
...
# extracting vars renaming and and creating df
...
Visualizing how the way people meet for the first time has changed over the years. Starting by creating two (2) charts to highlight important pattern(s).
...
base_graph <- base_graph + theme(legend.position = "none")
base_graph

#save
ggsave(base_graph, filename = "images/base_graph.png", width = 6, height = 4, dpi = 300)
```
...
# save 
ggsave(graph_1, filename = "images/graph_1.png", width = 6, height = 4, dpi = 300)
```
...
# save 
ggsave(graph_2, filename = "images/graph_2.png", width = 6, height = 4, dpi = 300)
```

Creating an alternative visualization of the same data to explore different patterns and to compare which one is more effective at conveying the story.
...
# save 
ggsave(graph_3, filename = "images/graph_3.png", width = 6, height = 4, dpi = 300)
```
...
Illustrating the relationship between a respondent's age and their partner's age, accounting for the gender of the respondent.
...
# save
ggsave(graph_4, filename = "images/graph_4.png", width = 6, height = 4, dpi = 300)
```

**Critiquing based on Tufte principles**: The color, the grid lines, and the relatively large data points are unnecessary. Based on data-inc-ration alone, the graph is not at par with what Tuft would have visioned for a effective and compelling visual aid. I think that the scatter plot would improve (from Tufte standards) from smaller lm lines, and smaller data points. Additionally, I would unbold the annotation/caption.

### 3. Politics and Dating

Exploring how the political affiliation of partners affects how couples meet and stay together.
...
# save
ggsave(graph_5, filename = "images/graph_5.png", width = 6, height = 4, dpi = 300)
```
...
### 4. Meeting Types and Relationship Outcomes
Exploring the relationship between meeting types and relationship outcomes (e.g. marriage, divorce, separation, breakup) can provide insights into how the context of meeting may influence the trajectory of relationships. I create two (2) pie charts to compare the distribution of meeting types between married and divorced/separated respondents. 
...
# save
ggsave(graph_7, filename = "images/graph_7.png", width = 8, height = 4, dpi = 300)
```
...
### 5. Applying Interactivity to Pie Charts and Temporal Trends

Testing manual and package-wise (`ggplotly`) implementation of interactivity.
...
# save as html
htmlwidgets::saveWidget(interactive_pie_mar, "images/interactive_pie_mar.html")
htmlwidgets::saveWidget(interactive_pie_divsep, "images/interactive_pie_divsep.html")
```

**Interactive Plot 2:**
```{r}
# Interactive Plot 2:
interactive_graph_3 <- ggplotly(graph_3)

# save as html
htmlwidgets::saveWidget(interactive_graph_3, "images/interactive_graph_3.html")

```

**Significance of Interactivity**: Interactivity in these visualizations allows readers to explore the data in a more engaging and intuitive way. For example, hovering over the pie chart slices reveals exact percentages and meeting types, making it easier to compare how married and divorced/separated couples met. Similarly, the line graph enables readers to focus on specific trends, such as the sharp rise of online meeting platforms after 2000. These features make the data more accessible and encourage readers to uncover insights on their own.

### 6. Data Table

Developing DT explorer to allow readers to better engage with the survey data on their own. 
...
# save as html
htmlwidgets::saveWidget(datatable(interactive_tab), "images/interactive_table.html")
```
